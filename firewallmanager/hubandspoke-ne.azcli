...
admin_user=azadmin
loc="northeurope"
rg=az-fw-routing-hubspoke-ne-rg
...
#test VMs
name=vm-ne-spoke1
az vm create --image ubuntults -g $rg -n $name --admin-password $admin_password --admin-username $admin_user -l $loc --public-ip-address "$name-pip" --vnet-name ne-spoke-vnet-01 --subnet servers --os-disk-size 30 --storage-sku Standard_LRS --size Standard_B1s --no-wait

name=vm-ne-spoke2
az vm create --image ubuntults -g $rg -n $name --admin-password $admin_password --admin-username $admin_user -l $loc --public-ip-address "$name-pip" --vnet-name ne-spoke-vnet-02 --subnet servers --os-disk-size 30 --storage-sku Standard_LRS --size Standard_B1s --no-wait

name=vm-ne-hub
az vm create --image ubuntults -g $rg -n $name --admin-password $admin_password --admin-username $admin_user -l $loc --public-ip-address "$name-pip" --vnet-name ne-hub-vnet --subnet servers --os-disk-size 30 --storage-sku Standard_LRS --size Standard_B1s --no-wait
...
az network route-table create -g $rg -n $rt -l $loc --disable-bgp-route-propagation false
az network route-table route create -n toInternet -g $rg --route-table-name $rt --address-prefix 0.0.0.0/0  --next-hop-type VirtualAppliance  --next-hop-ip-address $fwIP
...
az network firewall ip-config create -f $fwName -n ipconfig --public-ip-address "$fwName-pip" -g $rg --vnet-name ne-hub-vnet
